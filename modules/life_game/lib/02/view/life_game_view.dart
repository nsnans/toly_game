    ...
    return Column(
      children: [
        ...
        StatusBar(
          ...
          onSpeedChange: (EvolveSpeed value) => game.frameEvolve.speed = value,
          speed: _speedNtf,
        ),
      ],
    );
...
class TransformWrapper extends StatelessWidget {
  ...
  void _onPointerSignal(PointerSignalEvent event) {
    ...
    if (event is PointerScrollEvent) {
      ...
      // widget.game.camera.viewfinder.anchor
      // widget.game.camera.viewfinder.zoom = newZoom;
      Viewfinder viewfinder = game.camera.viewfinder;
      Matrix4 m = viewfinder.transform.transformMatrix;

      double dx = m.getTranslation().x;
      double dy = m.getTranslation().y;
      double dx1 = viewfinder.transform.x;
      double dy1 = viewfinder.transform.y;

      Offset origin = event.localPosition;

      Vector2 center = Vector2(dx1, dy1)
          // viewfinder.globalToLocal(Vector2(origin.dx, origin.dy))-
          //     Vector2(dx1, dy1)

          ;
      print("===(${dx},${dy})===(${dx1},${dy1})====v(${center.x},${center.y})===${curZoom}=");

      ///curZoom

      // Offset center = (origin - Offset(dx, dy)) / curZoom;

      // widget.game.camera.moveTo(center/curZoom);
